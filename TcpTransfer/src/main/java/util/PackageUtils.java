...
public class PackageUtils {
	...
	// 与参数为"xxx.yyy.*"无区�?
    //	TODO fix bug
    public static String[] getResourceInPackage(String pkgName) throws IOException {
    	Set<String> vResult = new HashSet<String>();
    	String[] pkgs = pkgName.split(",");
    	for(String pkg : pkgs) {
	        String packageOnly = pkg;
	        boolean recursive = false;
	        if (pkg.endsWith(".*")) {
	            packageOnly = pkg.substring(0, pkg
	                    .lastIndexOf(".*"));
	            recursive = true;
	        }
	        
	        if (packageOnly.endsWith("/")) {
	        	packageOnly = packageOnly.substring(0, pkg.length() - 1 );
	        }
	        ...
	        while (dirs.hasMoreElements()) {
	            ...
	            if ("file".equals(protocol)) {
	                getResourceInDirPackage(packageOnly,
	                        ...
	                        vResult);
	                //获得URI协议的File路径下所有文件的File的包路径  -wang
	            } 
	                ...
	                while (entries.hasMoreElements()) {
	                    ...
	                    if (name.startsWith(packageDirName)) {
	                        ...
	                        if ( logger.isDebugEnabled() ) {
	                            logger.debug("PackageUtils: Package name is " + pkg);
	                        }
	
	                        if ((idx != -1) || recursive) {
	                            // it's not inside a deeper dir
	                            if (!entry.isDirectory()) {
	                                String resName = name.substring(pkg.length() + 1);
	                            	vResult.add(pkg + "." + resName);
	                            }
	                        }
	                    }
	                }
